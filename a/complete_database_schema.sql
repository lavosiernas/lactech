-- =====================================================
-- SISTEMA LACTECH - ESQUEMA COMPLETO DO BANCO DE DADOS
...
CREATE TABLE IF NOT EXISTS farms (
    ...
    cnpj VARCHAR(18), -- Pode ser CPF ou CNPJ
...
CREATE TABLE IF NOT EXISTS users (
    id UUID PRIMARY KEY, -- Será o mesmo ID do Supabase Auth
...
CREATE TABLE IF NOT EXISTS animals (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    farm_id UUID NOT NULL REFERENCES farms(id) ON DELETE CASCADE,
    identification VARCHAR(50) NOT NULL, -- Brinco, chip, etc.
    name VARCHAR(100),
    breed VARCHAR(100),
    birth_date DATE,
    weight DECIMAL(6,2),
...
CREATE TABLE IF NOT EXISTS quality_tests (
    ...
    somatic_cell_count INTEGER, -- CCS
    total_bacterial_count INTEGER, -- CBT
...
-- Índice único parcial para CNPJ (apenas quando não é nulo ou vazio)
...
-- Política para permitir inserção durante o primeiro acesso
CREATE POLICY farms_insert_policy ON farms
    FOR INSERT TO authenticated
    WITH CHECK (true);

-- Política para seleção - usuários só veem sua própria fazenda
...
-- Política para atualização - apenas proprietários e gerentes
...
CREATE POLICY users_insert_policy ON users
    FOR INSERT TO authenticated
    WITH CHECK (
        -- Permite inserção durante primeiro acesso ou por proprietários/gerentes
        farm_id IN (
...
-- Animais
CREATE POLICY animals_policy ON animals
...
-- Produção de Leite
CREATE POLICY milk_production_policy ON milk_production
    FOR ALL TO authenticated
    USING (
        farm_id IN (
            SELECT farm_id FROM users WHERE id = auth.uid()
...
-- Testes de Qualidade
CREATE POLICY quality_tests_policy ON quality_tests
    FOR ALL TO authenticated
    USING (
        farm_id IN (
            SELECT farm_id FROM users WHERE id = auth.uid()
...
CREATE POLICY animal_health_records_policy ON animal_health_records
    FOR ALL TO authenticated
    USING (
        farm_id IN (
            SELECT farm_id FROM users WHERE id = auth.uid()
...
-- Tratamentos
CREATE POLICY treatments_policy ON treatments
    FOR ALL TO authenticated
    USING (
        farm_id IN (
            SELECT farm_id FROM users WHERE id = auth.uid()
...
CREATE POLICY user_access_requests_policy ON user_access_requests
    FOR ALL TO authenticated
    USING (
        farm_id IN (
            SELECT farm_id FROM users WHERE id = auth.uid()
...
-- =====================================================
-- FUNÇÕES RPC PARA O SISTEMA
-- =====================================================

-- Remover funções existentes para evitar conflitos de parâmetros
...
-- Função para registrar produção de leite
...
-- Inserir alguns dados de exemplo para testes (comentado por padrão)
/*
-- Exemplo de fazenda
INSERT INTO farms (name, owner_name, city, state, animal_count, daily_production, setup_completed)
VALUES ('Fazenda Exemplo', 'João Silva', 'Uberlândia', 'MG', 50, 500.00, true);

-- Exemplo de usuário (necessário ter o ID do Supabase Auth)
-- INSERT INTO users (id, farm_id, name, email, role)
-- VALUES ('uuid-do-supabase-auth', 'uuid-da-fazenda', 'João Silva', 'joao@exemplo.com', 'proprietario');
*/
...
-- 1. Estrutura completa de tabelas para o sistema LacTech
-- 2. Políticas RLS robustas para segurança multi-tenant
-- 3. Funções RPC para operações seguras
-- 4. Índices para performance
-- 5. Triggers para auditoria
-- 6. Constraints para integridade dos dados

-- Para usar este esquema:
-- 1. Execute este script no seu banco PostgreSQL/Supabase
-- 2. Configure as variáveis de ambiente no frontend
-- 3. Teste o fluxo de primeiro acesso
